...
function initializeServiceWorker() {
  ...
  if ("serviceWorker" in navigator) {
    ...
    try {
      ...
      window.addEventListener('load', swFunc);
...
async function getRecipes() {
  ...
  let localInputRecipes = localStorage.getItem('recipes');
  if (localInputRecipes) {
    return JSON.parse(localInputRecipes);
  }
  ...
  return new Promise(async (resolve, reject) => {
    for (const url of RECIPE_URLS) {
      try {
        ...
        var urlJSON = await fetchedUrl.json();
        recipesArr.push(urlJSON);
